import numpy as np
import time
from pyfilter.kalman import KalmanFilter
...
# Inicjalizacja stanu początkowego i kowariancji
initial_state_mean = np.zeros(6) # [x, y, vx, vy, ax, ay] - początkowy stan
initial_state_covariance = np.eye(6) * 100.0 # Początkowa kowariancja (duża niepewność)


# ---- Inicjalizacja Filtra Kalmana z PyFilter ----
kf = KalmanFilter(state_transition=F,
                   observation_model=H,
                   process_noise=Q,
                   observation_noise=R,
...
try:
    while True:
        ...
        # 1. Predykcja Filtra Kalmana (używamy predict z pyfilter)
        kf.predict()
        ...
        # 4. Aktualizacja Filtra Kalmana (używamy update z pyfilter)
        if current_time - last_gps_time >= gps_interval and gps_x is not None and gps_y is not None and gps_vx is not None and gps_vy is not None:
            ...
            z_pomiar = np.array([None, None, None, None, accel_ax, accel_ay]) # Tylko pomiar z akcelerometru (None dla GPS) - **ważne: PyFilter może wymagać numerycznego wektora pomiarów, nawet częściowo nieaktualnego. W tym prostym przykładzie, przekazujemy aktualne odczyty akcelerometru, a wartości GPS mogą być 'stare' lub None, jeśli nie są dostępne.**
            z_pomiar_fillna = np.array([gps_x if gps_x is not None else kf.state.mean[0], # Użyj ostatniej estymaty, jeśli brak GPS
                                        gps_y if gps_y is not None else kf.state.mean[1],
                                        gps_vx if gps_vx is not None else kf.state.mean[2],
                                        gps_vy if gps_vy is not None else kf.state.mean[3],
                                        accel_ax, accel_ay]) # Wektor pomiarów z Akcelerometru i ewentualnie ostatniej estymaty GPS
            kf.update(z_pomiar_fillna) # Aktualizacja filtra z danymi Akcelerometru i ewentualnie ostatnią estymatą GPS


        # 5. Wyjście Wyników (pozycja i prędkość z filtra Kalmana)
        pozycja_x = kf.state.mean[0]
        pozycja_y = kf.state.mean[1]
        predkosc_x = kf.state.mean[2]
        predkosc_y = kf.state.mean[3]
        przyspieszenie_x = kf.state.mean[4]
        przyspieszenie_y = kf.state.mean[5]
